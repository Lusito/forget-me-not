...
export function CleanupTab() {
    const rows = CLEANUP_SETTINGS.map(([i18n, startup, startupRules, domainLeave, manualCleanup, manualCleanupRules]) => {
        return <tr>
            <td data-i18n={i18n} />
            <td>{startup && <SettingsCheckbox key={startup} enabledBy="startup.enabled" i18n="setting_type_gray?title" />}</td>
            <td>{startupRules && <SettingsCheckbox key={startupRules} enabledBy={`startup.enabled ${startup}`} i18n="setting_type_apply_rules?title" />}</td>
            <td>{domainLeave && <SettingsCheckbox key={domainLeave} enabledBy="domainLeave.enabled" i18n="setting_type_forget?title" />}</td>
        </tr>;
    });

    const legend = browserInfo.mobile && <ul class="settings_legend">
        <li><img src="../icons/tabs/power.svg" /> <span data-i18n="setting_type_gray@title" /></li>
        <li><img src="../icons/tabs/shield.svg" /> <span data-i18n="setting_apply_rules" /></li>
        <li><img src="../icons/tabs/exit.svg" /> <span data-i18n="setting_type_forget@title" /></li>
    </ul>;
        ...
        <table class="settings_table">
            <thead>
                <tr>
                    ...
                    <th><img src="../icons/tabs/power.svg" data-i18n="setting_type_gray?title" /></th>
                    <th><img src="../icons/tabs/shield.svg" data-i18n="setting_type_apply_rules?title" /></th>
                    <th><img src="../icons/tabs/exit.svg" data-i18n="setting_type_forget?title" /></th>
                </tr>
                <tr>
                    <th>Enable</th>
                    <th><SettingsCheckbox key="startup.enabled" i18n="setting_type_gray?title" /></th>
                    <th />
                    <th><SettingsCheckbox key="domainLeave.enabled" i18n="setting_type_forget?title" /></th>
                </tr>
            </thead>
            <tbody>{rows}</tbody>
        </table>
